    ...
    protected void buildChattingData(final Context context, BaseHolder baseHolder, final FromToMessage detail, int position) {
        ...
        if(message != null) {
            if(message.withDrawStatus) {
                ...
                Glide.with(context).load(message.richTextPicUrl+"?imageView2/0/w/200/h/140").apply(new RequestOptions().centerCrop().error(R.drawable.kf_image_download_fail_icon)).into(holder.getImageView());
