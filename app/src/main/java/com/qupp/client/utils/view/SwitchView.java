...
public class SwitchView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		paint.setColor(isOn ? 0xffDD0000 : 0xffe3e3e3);//开启和关闭时的背景色
		...
		// draw shadow
		paint.setStyle(Style.FILL);
		//paint.setColor(0xff333333);//边框阴影颜色
		...
		paint.setColor(isOn ? 0xffDD0000 : 0xffbfbfbf);
		canvas.drawPath(bPath, paint);
	...
	private OnStateChangedListener listener = new OnStateChangedListener() {
		...
		public void toggleToOn(View view) {
			...
			Log.v("state1", view.getId()+"true");
			clistener.listener(view, true);
		}

		@Override
		public void toggleToOff(View view) {
			toggleSwitch(STATE_SWITCH_OFF);
			Log.v("state1", view.getId()+"false");
			clistener.listener(view, false);
		}
	};
	...
	public static interface ChangeListener{
		void listener(View v, boolean ischecked);
	}
