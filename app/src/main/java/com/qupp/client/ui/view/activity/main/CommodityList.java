...
public class CommodityList extends BaseActivity implements AbsListView.OnScrollListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        linear.setPadding(0, MyApplication.getStateBar(CommodityList.this), 0, 0);
        setStateColor(false);
    ...
    private void initAdapter(List<EntityForSimple> data) {
       ...
       //mRecyclerView.setOnItemClickListener((parent, view, position, id) -> NewCommodityDetailsActivity.startActivityInstance(CommodityList.this, data.get(position).getStatus(), data.get(position).getAuctionId(), data.get(position).getTopPrice(),data.get(position).getGoodsName()));
    ...
    public static void startActivityInstance(Context activity, String title, String goodsId) {
        activity.startActivity(new Intent(activity, CommodityList.class)
                .putExtra("title", title)
                .putExtra("goodsId",goodsId));
    }

    public static void startActivityInstance1(Context activity, String title, String goodsId) {
        activity.startActivity(new Intent(activity, CommodityList.class)
                .putExtra("title", title)
    ...
    private void initData() {
        try {
            ApiUtil.getApiService().newListDate(current + "", size + "","4",goodsId).enqueue(new Callback<MessageForSimple>() {
                @Override
                    ...
                    try {
                        if (entity.getCode().equals("0")) {
                            if (current == 1) {
                                ...
                                if (entity.getData().getRecords().size() == 0) {
                                    Toast.makeText(CommodityList.this, "没有更多了", Toast.LENGTH_LONG).show();
                                    mHasRequestedMore = true;
                                }else{
                                    mHasRequestedMore = false;
                                }
