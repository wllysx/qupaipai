...
public class KnowActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        linear.setPadding(0, MyApplication.getStateBar(KnowActivity.this), 0, 0);
        setStateColor(false);
        initView();
        getData();
    ...
    private void getData() {
        ApiUtil.getApiService().ruleByKey("userKnow").enqueue(new Callback<MessageForSimple>() {
            @Override
            public void onResponse(Call<MessageForSimple> call, Response<MessageForSimple> response) {
                MessageForSimple entity = response.body();
                try {
                    if (entity.getCode().equals("0")) {
                        tvLongimage.setText(Html.fromHtml(entity.getData().getContent(), new MyImageGetter(KnowActivity.this, tvLongimage), null));
                    } else {
    ...
    public static void startActivityInstance(Activity activity) {
        activity.startActivity(new Intent(activity, KnowActivity.class));
    }
