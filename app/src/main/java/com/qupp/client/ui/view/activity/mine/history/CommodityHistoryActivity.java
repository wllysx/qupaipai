package com.qupp.client.ui.view.activity.mine.history;
...
public class CommodityHistoryActivity extends BaseActivity implements AbsListView.OnScrollListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        linear.setPadding(0, MyApplication.getStateBar(CommodityHistoryActivity.this), 0, 0);
        setStateColor(false);
        initView();
    ...
    private void initAdapter(List<EntityForSimple> data) {
       ...
       mRecyclerView.setOnItemClickListener((parent, view, position, id) -> NewCommodityDetailsActivity.startActivityInstance(CommodityHistoryActivity.this, data.get(position).getStatus(), data.get(position).getAuctionId(), data.get(position).getTopPrice(),data.get(position).getGoodsName()));
    ...
    public static void startActivityInstance(Context activity) {
        activity.startActivity(new Intent(activity, CommodityHistoryActivity.class));
    }
    ...
    private void initData() {
        try {
            ApiUtil.getApiService().listHistory(current + "", size + "").enqueue(new Callback<MessageForSimple>() {
                @Override
                    ...
                    try {
                        if (entity.getCode().equals("0")) {
                            if (current == 1) {
                                ...
                                if (entity.getData().getRecords().size() == 0) {
                                    Toast.makeText(CommodityHistoryActivity.this, "没有更多了", Toast.LENGTH_LONG).show();
                                    mHasRequestedMore = true;
                                }else{
                                    mHasRequestedMore = false;
                                }
